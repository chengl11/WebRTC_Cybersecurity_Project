...
Similar to the "WebRTC Control" plug-in, when users open Chrome, all the WebRTC components of will be disabled. When a site wants to use one of the WebRTC components, the console should report an error. Our product could capture these errors and analyze the results. Therefore, we can build our products around these errors.
